package com.theruipu.policy.register;
...
public class FiveminuteRegisterRanking implements Serializable {
	...
	public void Count() throws IOException, SQLException {
		...
		//SparkConf conf = new SparkConf().setAppName("FiveminuteRegisterRanking").setMaster("local");
		String Environment = Getconfig.Getproperty("env");
		SparkConf conf = new SparkConf();
		if(Environment.equals("PROD")) {
			conf.setAppName("FiveminuteRegisterRanking");
		}
		else {
			conf.setAppName("FiveminuteRegisterRanking").setMaster("local");
		}
		...
		JavaRDD<String> linesLogin = lines.filter(new Function<String, Boolean>() {
			public Boolean call(String s) throws Exception {
				...
				if (tmp.Success == 2) {
					List<String>  loginURL=Geturls.getkeylist("register");
		...
		if(wordMap.size()<50) {
		  ...
		  for(Integer i=0;i<size;i++) {
				...
				MysqlUtils.InsertFiveminuteRegisterRanking(project, Timestr, wordMap.get(i).getKey(),  wordMap.get(i).getValue());
