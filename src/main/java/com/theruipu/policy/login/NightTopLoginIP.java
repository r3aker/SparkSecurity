...
public class NightTopLoginIP implements Serializable {
	private static final long serialVersionUID = 523113L;
		...
		if(Environment.equals("PROD")) {
			conf.setAppName("TopLoginFailIP");
		}
		else {
			conf.setAppName("TopLoginFailIP").setMaster("local");
		}
		...
		JavaPairRDD<String, Integer> IPandNumber1 = linesLogin.mapToPair(new PairFunction<String, String, Integer>() {
			private static final long serialVersionUID = 1L;

			public Tuple2<String, Integer> call(String s) {
				AccessLog tmp = new AccessLog(s);
				if (tmp.Success == 2 && isRightTime(tmp.getTime())) {
		...
		for (Tuple2<String, Integer> a : last.take((int)size)) {
			MysqlUtils.InsertNightLogin(project,Timestr,a._1(),a._2());
