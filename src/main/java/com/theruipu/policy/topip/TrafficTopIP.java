...
public class TrafficTopIP implements Serializable {
	...
	public void Count()  throws IOException, SQLException {
		...
		JavaPairRDD<String, Integer> IPAndResponse = lines.mapToPair(new PairFunction<String, String, Integer>() {
			private static final long serialVersionUID = 1L;

			public Tuple2<String, Integer> call(String s) {
				AccessLog tmp = new AccessLog(s);
				if (tmp.Success == 2) {
					return new Tuple2<String, Integer>(tmp.getClientIP(),tmp.getResponseSize());
				} else {
		...
		JavaPairRDD<String, Integer> IPAndResponseFilter = IPAndResponse.filter(new Function<Tuple2<String, Integer>, Boolean>() {
			private static final long serialVersionUID = 1L;
		...
		JavaPairRDD<String, Integer> IPAndTotalResponse = IPAndResponseFilter
		...
		JavaPairRDD<Integer, String> wordtop1 = IPAndTotalResponse
		...
		for (Tuple2<String, Integer> a : lastlast.take((int)size)) {
			MysqlUtils.InsertTrafficTopIP(project,Timestr,a._1(),a._2());
		}
