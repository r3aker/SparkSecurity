package com.theruipu.policy.sendmessage;
...
public class TopIPSendMessage implements Serializable {
	...
	public void Count() throws IOException, SQLException {
		...
		if(Environment.equals("PROD")) {
			conf.setAppName("TopIPSendMessage");
		}
		else {
			conf.setAppName("TopIPSendMessage").setMaster("local");
		}
		...
		JavaRDD<String> linesSendmessage = lines.filter(new Function<String, Boolean>() {
			public Boolean call(String s) throws Exception {
				AccessLog tmp = new AccessLog(s);
				if (tmp.Success == 2) {
					List<String>  loginURL=Geturls.getkeylist("sendmessage");
...
//				if(s.contains(" /entrance/user/sendphonecode/json ")) {
//					return true;
//				}
//				else if (s.contains(" /register/rest/message ")){return true;}
//				else if (s.contains(" /verificationCode/generateForCurrentUser ")){return true;}
//				else {return false;}
			}
		});
		
		JavaPairRDD<String, Integer> IPandNumber1 = linesSendmessage.mapToPair(new PairFunction<String, String, Integer>() {
		...
		JavaPairRDD<Integer, String> wordtop1 = IPAndNumber
		...
		for (Tuple2<String, Integer> a : last.take((int)size)) {
			MysqlUtils.InsertTopIPSendMessage(project,Timestr,a._1(),a._2());
