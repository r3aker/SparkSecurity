...
public class FiveminuteSendmessageRanking implements Serializable {
	...
	public void Count()  throws IOException, SQLException {
		...
		//SparkConf conf = new SparkConf().setAppName("FiveminuteSendmessageRanking").setMaster("local");
		String Environment = Getconfig.Getproperty("env");
		SparkConf conf = new SparkConf();
		if(Environment.equals("PROD")) {
			conf.setAppName("FiveminuteSendmessageRanking");
		}
		else {
			conf.setAppName("FiveminuteSendmessageRanking").setMaster("local");
		}
...
//				else {return false;}
		...
		// {20180509092500_222.222.44.234,1}
		// 返回每分钟，每个IP的登录次数
	...
	Map<String,Integer> ssss=new HashMap<String,Integer>();
		...
		if(wordMap.size()<50) {
		  ...
		  for(Integer i=0;i<size;i++) {
				...
				MysqlUtils.InsertFiveminuteSendmessageRanking(project, Timestr, wordMap.get(i).getKey(),  wordMap.get(i).getValue());
