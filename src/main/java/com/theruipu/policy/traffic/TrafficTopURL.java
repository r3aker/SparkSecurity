package com.theruipu.policy.traffic;
...
public class TrafficTopURL implements Serializable {
	...
	public void Count() throws IOException, SQLException {
		...
		JavaPairRDD<String, Integer> URLAndResponse = lines.mapToPair(new PairFunction<String, String, Integer>() {
			private static final long serialVersionUID = 1L;

			public Tuple2<String, Integer> call(String s) {
				AccessLog tmp = new AccessLog(s);
				if (tmp.Success == 2) {
					return new Tuple2<String, Integer>(tmp.getURL(),tmp.getResponseSize());
				} else {
		...
		JavaPairRDD<String, Integer> URLAndResponseFilter = URLAndResponse.filter(new Function<Tuple2<String, Integer>, Boolean>() {
			private static final long serialVersionUID = 1L;
		...
		JavaPairRDD<String, Integer> URLAndTotalResponse = URLAndResponseFilter
		...
		JavaPairRDD<Integer, String> wordtop1 = URLAndTotalResponse
		...
		JavaPairRDD<String, Integer> lastlast = last
				.mapToPair(new PairFunction<Tuple2<String,Integer>, String, Integer>() {
					public Tuple2<String, Integer> call(Tuple2<String,Integer> item) throws Exception {
						return new Tuple2<String, Integer>(item._1(),item._2()/1048576);
					}
				});
		...
		for (Tuple2<String, Integer> a : lastlast.take((int)size)) {
			MysqlUtils.InsertTrafficTopURL(project,Timestr,a._1(),a._2());
		}
