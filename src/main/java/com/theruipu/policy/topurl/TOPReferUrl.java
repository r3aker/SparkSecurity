...
public class TOPReferUrl implements Serializable {
	...
	public void Count() throws SQLException {
		...
		if (Environment.equals("PROD")) {
			conf.setAppName("TopReferUrl");
		} else {
			conf.setAppName("TopReferUrl").setMaster("local");
		}
		...
		JavaPairRDD<String, Integer> IPandNumber1 = lines.mapToPair(new PairFunction<String, String, Integer>() {
			public Tuple2<String, Integer> call(String s) {
				...
				if (tmp.Success == 2) {
					return new Tuple2<String, Integer>(tmp.getRefer(), 1);
				} else {
		...
		for (Tuple2<String, Integer> a : last.take((int) size)) {
			MysqlUtils.InsertTopReferURL(project, Timestr, a._1(), a._2());
		}
