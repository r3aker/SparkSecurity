...
public class TOPUrlNoStaticFile implements Serializable {
	...
	public  void Count() throws SQLException {
		...
		// SparkConf conf = new SparkConf().setAppName("TOPUrl").setMaster("local");
			...
			if(Environment.equals("PROD")) {
				conf.setAppName("TOPUrlNoStatic");
			}
			else {
				conf.setAppName("TOPUrlNoStatic").setMaster("local");
			}
			...
			public Tuple2<String, Integer> call(String s) {
				...
				if(tmp.Success==2) {
					return new Tuple2<String, Integer>(tmp.getURL(), 1);
						...
						return new Tuple2<String, Integer>("/test.ssssssshtml", 1);
					}
			}
		...
		JavaPairRDD<String, Integer> IPandNumber1Filter = IPandNumber1.filter(new Function<Tuple2<String, Integer>, Boolean>() {
			...
			public Boolean call(Tuple2<String, Integer> s) {
				if (s._1().equals("/test.ssssssshtml")) {
					return false;
				}
		...
		JavaPairRDD<String, Integer> URLAndNumber = IPandNumber1Filter.reduceByKey(new Function2<Integer, Integer, Integer>() {
		...
		for(Tuple2<String,Integer> a : last.take((int)size)) {
			MysqlUtils.InsertTopURLNoStatic(project,Timestr,a._1(),a._2());
		}
